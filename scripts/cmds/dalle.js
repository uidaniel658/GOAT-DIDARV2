...
module.exports = {
  ...
  onStart: async({ api, event, args }) => {
    ...
    if (!prompt) return api.sendMessage("📛| Wrong Format. 🎖️ | Use: 17/18 years old boy/girl watching football match on TV with 'Dipto' and '69' written on the back of their dress, 4k", event.threadID, event.messageID);
    try {
...
const randomCookie = cookies[Math.floor(Math.random() * cookies.length)];
      const wait = api.sendMessage("please w8 🦆", event.threadID);
...
const imageUrls = response.data.imgUrls || [];
   ...
   api.sendMessage({ body: `🎖️ | Here's Your Generated Photo 🦆`, attachment: images }, event.threadID, event.messageID);
    } catch (error) {
      console.error(error);
